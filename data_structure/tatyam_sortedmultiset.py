...
class SortedMultisetInt(SortedMultiset[int]):
    def _build(self, a: Optional[List[int]]=None) -> None:
        "Evenly divide `a` into buckets."
        if a is None: a = list(self)
        size = len(a)
        bucket_size = int(ceil(sqrt(size / self.BUCKET_RATIO)))
        self.a = [array('i', a[size * i // bucket_size : size * (i + 1) // bucket_size]) for i in range(bucket_size)]
